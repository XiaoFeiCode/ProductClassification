...
这个模块可以放在“智能商品发布”项目中介绍。完整项目可以包含商品图片生成标题、商品标题预测类目、商品视频简介生成等能力。本仓库聚焦其中的商品类目预测模块。

该模块本质上是一个文本多分类任务。输入是商品标题，输出是商品所属三级类目。由于三级类目可以映射到二级类目和一级类目，所以模型只需要预测三级类目，就能得到完整的类目路径。

实现流程上，首先从业务数据库中抽取商品名称和类目信息，构造 `spu_name -> base_category3` 的训练样本。由于业务类目存在长尾分布，热门类目样本较多、冷门类目样本较少，因此在数据处理阶段对热门类目做下采样，并使用大模型生成部分冷门类目的商品标题样本，提升类别分布的均衡性。

模型选择的是 `bert-base-chinese`，基于 Hugging Face Transformers 的 `BertForSequenceClassification` 完成微调。训练时采用全参数微调、混合精度训练、Warmup 和线性学习率衰减，并根据验证集 F1 进行早停。业务版本在约 50 万条样本、约 1000 个类别的数据规模下，单张 V100 32GB 训练约 1 小时，F1 分数约 76%。

这个模块的价值在于降低运营人员手动选择类目的成本，提升商品上架效率，并减少类目选择不一致导致的审核和检索问题。
